    ...
    >
      ...
      <section className="bg-blue-900 py-16 text-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="text-blue-100 max-w-xl mx-auto">Find an ADTEMPCO enterprise near you. We are growing to serve you better.</p>
        </div>
      </section>
              ...
              <div className="space-y-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {['All', 'Retail', 'Hospitality', 'Distribution', 'Business'].map(filter => (
                    <button 
                      key={filter}
                      className={`px-4 py-2 rounded-full text-xs font-bold transition-all ${filter === 'All' ? 'bg-blue-600 text-white shadow-md' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
                    >
                      {filter}
                    </button>
                  ))}
            ...
            <div className="space-y-4 overflow-y-auto max-h-[600px] pr-2 custom-scrollbar">
              {branches.map((branch, index) => (
                <motion.div 
                  ...
                  className="bg-white p-6 rounded-2xl shadow-md border border-gray-100 hover:border-blue-600 transition-all cursor-pointer group"
                >
                  <div className="flex justify-between items-start mb-4">
                    <h3 className="font-bold text-blue-900 group-hover:text-blue-600 transition-colors">{branch.name}</h3>
                    <span className="bg-blue-50 text-blue-600 px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider">
                      {branch.type}
                    </span>
                  </div>
                  <div className="space-y-2 text-sm text-gray-600">
                    <p className="flex items-start"><MapPin className="h-4 w-4 mr-2 text-blue-600 shrink-0 mt-0.5" /> {branch.address}</p>
                    <p className="flex items-center"><Phone className="h-4 w-4 mr-2 text-blue-600 shrink-0" /> {branch.phone}</p>
                    <p className="flex items-center"><Clock className="h-4 w-4 mr-2 text-blue-600 shrink-0" /> {branch.hours}</p>
                  </div>
                </motion.div>
              ))}
            </div>
